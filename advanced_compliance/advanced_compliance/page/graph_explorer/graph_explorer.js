...
class GraphExplorer {
    ...
    setup_page() {
        // Add page actions
        this.page.set_primary_action(__('Refresh'), () => this.load_graph(), 'refresh');

        this.page.add_menu_item(__('Rebuild Graph'), () => this.rebuild_graph());
        this.page.add_menu_item(__('Export as Image'), () => this.export_image());
        this.page.add_menu_item(__('View Statistics'), () => this.show_statistics());
    }
                        ...
                        <div class="details-panel">
                            ...
                            <div class="mt-3">
                                ...
                                <div id="entity-relationships" class="p-3 border rounded">
                                    <p class="text-muted">${__('Select an entity to see relationships')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        frappe.call({
            ...
            callback: function(r) {
                ...
                if (r.message && r.message.length > 0) {
                    const actual_types = [...new Set(r.message.map(e => e.entity_type))];
                ...
                if (!legend_html) {
                    legend_html = '<span class="text-muted">' + __('No entities in graph. Click Menu → Rebuild Graph.') + '</span>';
                }
    ...
    async load_graph() {
        ...
        try {
            const data = await frappe.call({
                method: 'advanced_compliance.advanced_compliance.knowledge_graph.query.get_visualization_data',
                args: {
    ...
    async on_node_click(node_id) {
        ...
        const details_html = `
            <div class="entity-info">
                ...
                <table class="table table-sm">
                    <tr><td><strong>${__('ID')}</strong></td><td>${node.id}</td></tr>
                    <tr><td><strong>${__('Type')}</strong></td><td>${node.group || '-'}</td></tr>
                </table>
        ...
        try {
            const rel_data = await frappe.call({
                method: 'advanced_compliance.advanced_compliance.knowledge_graph.query.get_entity_neighbors',
                args: {
                    entity_name: node_id,
                    direction: 'both',
                    max_depth: 1
    ...
    show_relationships(relationships) {
        if (!relationships.length) {
            $('#entity-relationships').html(`<p class="text-muted">${__('No relationships found')}</p>`);
            return;
        }
        ...
        if (edge) {
            $('#entity-details').html(`
                <div class="edge-info">
                    ...
                    <table class="table table-sm">
                        <tr><td><strong>${__('Type')}</strong></td><td>${edge.label || '-'}</td></tr>
                        <tr><td><strong>${__('From')}</strong></td><td>${edge.from}</td></tr>
                        <tr><td><strong>${__('To')}</strong></td><td>${edge.to}</td></tr>
                    </table>
                </div>
            `);
        }
    ...
    clear_selection() {
        this.selected_entity = null;
        $('#entity-details').html(`<p class="text-muted">${__('Click on a node to see details')}</p>`);
        $('#entity-relationships').html(`<p class="text-muted">${__('Select an entity to see relationships')}</p>`);
    }
    ...
    async rebuild_graph() {
        frappe.confirm(
            __('This will rebuild the entire knowledge graph from scratch. Continue?'),
            async () => {
                frappe.show_progress(__('Rebuilding Graph'), 0, 100, __('Starting...'));

                try {
                    const result = await frappe.call({
                        method: 'advanced_compliance.advanced_compliance.knowledge_graph.sync.rebuild_graph'
                    });
    ...
    async show_statistics() {
        try {
            const stats = await frappe.call({
                method: 'advanced_compliance.advanced_compliance.knowledge_graph.query.get_graph_statistics'
            });
                        ...
                        options: `
                            <div class="row">
                                ...
                                <div class="col-md-4 text-center">
                                    ...
                                    <p class="text-muted">${__('Relationships')}</p>
                                </div>
                                <div class="col-md-4 text-center">
                                    <h2>${s.total_paths}</h2>
                                    <p class="text-muted">${__('Computed Paths')}</p>
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <ul>${entity_breakdown || '<li>No entities</li>'}</ul>
                                </div>
                                <div class="col-md-6">
                                    <h6>${__('Relationships by Type')}</h6>
                                    <ul>${rel_breakdown || '<li>No relationships</li>'}</ul>
                                </div>
                            </div>
                        `
                    }]
