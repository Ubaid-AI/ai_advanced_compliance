...
class AskCompliance {
		...
		frappe.call({
			...
			callback: function(r) {
				if (r.message && r.message.enable_nl_queries) {
					...
					$(self.page.main).find('.ai-status').html(
						`<span class="text-success"><i class="fa fa-check-circle"></i> ${__('AI Enhanced')}</span>`
					);
				} else {
					$(self.page.main).find('.ai-status').html(
						`<span class="text-muted"><i class="fa fa-info-circle"></i> ${__('Rule-based mode')}</span>`
					);
				}
			}
		});
	...
	setup_page() {
			...
			<div class="ask-compliance-container" style="max-width: 900px; margin: 0 auto; padding: 20px;">
				<div class="search-section mb-4">
					...
					<div class="mt-2 ai-status">
						<span class="text-muted"><i class="fa fa-spinner fa-spin"></i> ${__('Checking AI status...')}</span>
					</div>
				</div>
		...
		// Event handlers
		$(this.page.main).find('.ask-btn').on('click', function() {
			self.ask_question();
		});

		$(this.page.main).find('.question-input').on('keypress', function(e) {
			if (e.which === 13) {
				self.ask_question();
			}
		});

		$(this.page.main).find('.example-btn').on('click', function() {
	...
	ask_question() {
		...
		$(this.page.main).find('.results-content').html(`
		...
		frappe.call({
			...
			callback: function(r) {
				if (r.message) {
					...
					if (self.ai_enabled && (!rule_result.success || rule_result.count === 0)) {
	...
	render_results(rule_result, ai_result) {
		...
		if (secondary_result && secondary_result.success && secondary_result !== primary_result) {
			...
			if (has_different_results) {
				console.log('Adding secondary result to HTML (count=' + secondary_result.count + ')');
				html += `<hr class="my-4">`;
	...
	render_single_result(result, is_primary) {
		console.log('render_single_result called for', result.query_type, 'is_primary:', is_primary);
		...
		if (result.query_type === 'llm') {
			badge = ' <span class="badge badge-primary"><i class="fa fa-robot"></i> AI</span>';
		} else {
			badge = ' <span class="badge badge-secondary"><i class="fa fa-cogs"></i> Pattern</span>';
		}
