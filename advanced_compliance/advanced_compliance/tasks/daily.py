...
def check_overdue_tests():
    ...
    for control in overdue_controls:
        ...
        alert = frappe.get_doc({
            ...
            "description": _("Control '{0}' has overdue testing. Due date was {1} ({2} days ago). Test frequency: {3}").format(
...
def send_control_owner_reminders():
    ...
    for owner, controls in owners.items():
        ...
        frappe.logger().info(
            f"Would send reminder to {owner} for {len(controls)} controls"
        )
