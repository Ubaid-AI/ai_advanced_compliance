        ...
        try:
            ...
            for ctrl_idx in control_indices:
                ...
                if control_name not in existing_controls:
                    risk_doc.append("mitigating_controls", {
...
def create_test_executions(controls, test_user):
    ...
    evidence_sets = [
        ...
        [
            {"evidence_type": "Screenshot", "description": "Revenue recognition system screenshot showing approved contracts"},
            {"evidence_type": "System Report", "description": "Contract listing report from Q3 with 25 sampled items highlighted"},
            {"evidence_type": "Approval Signature", "description": "Manager approval signatures on revenue memos"}
        ],
        # Evidence for Ineffective - Minor test
        [
            {"evidence_type": "Transaction Log", "description": "Journal entry approval log showing 2 late approvals"},
        ...
        [
            {"evidence_type": "System Report", "description": "Vendor master change report showing 5 exceptions"},
            {"evidence_type": "Screenshot", "description": "Missing callback documentation examples"},
            {"evidence_type": "Other", "description": "Interview notes with AP team regarding verification process"}
        ],
        # Evidence for Ineffective - Material test
        [
            {"evidence_type": "Configuration Export", "description": "Access certification completion rates by department"},
            ...
            {"evidence_type": "Screenshot", "description": "HR termination report vs. IT access report comparison"}
        ],
        # Evidence for Not Applicable test
        [
            {"evidence_type": "Email", "description": "Email from control owner confirming control under redesign"},
    ...
    for i, data in enumerate(test_data):
        ...
        existing = frappe.db.exists(
            "Test Execution",
            {"control": data["control"], "test_date": data["test_date"]}
        )
        if not existing:
            doc = frappe.get_doc({
...
def create_deficiencies(controls, test_executions, test_user):
    ...
    deficiency_data = [
        ...
        },
        {
            ...
            "test_execution": test_executions[4] if len(test_executions) > 4 else (test_executions[0] if test_executions else None),
            "severity": "Control Deficiency",
            "status": "Closed",
        ...
        },
        {
            ...
            "test_execution": test_executions[2] if len(test_executions) > 2 else (test_executions[0] if test_executions else None),
            "severity": "Significant Deficiency",
            "status": "Closed",
    ...
    for data in deficiency_data:
        ...
        existing = frappe.db.exists(
            "Deficiency",
            {"control": data["control"], "description": data["description"][:100]}
        )
        if not existing:
            doc = frappe.get_doc({
                "doctype": "Deficiency",
...
def create_control_evidence(controls, capture_rules, company, test_user):
    ...
    for rule_name in capture_rules:
        rule_doctype = frappe.db.get_value(
            "Evidence Capture Rule", rule_name, "source_doctype"
        )
        if rule_doctype:
    ...
    version_data = [
        {"version": 1, "modified_by": test_user, "changes": "Initial creation"}
    ]
    ...
    for data in evidence_data:
        ...
        if not existing:
            ...
            if data.get("capture_rule"):
                ...
                print(f"Created evidence {doc.name} WITHOUT capture_rule (no matching rule for {data['source_doctype']})")
        else:
...
def create_compliance_alerts(controls, risks, deficiencies):
    ...
    alert_data = [
        ...
        },
        {
            ...
            "detection_details": json.dumps({
                "expected_date": str(add_days(nowdate(), -15)),
                "days_overdue": 15
            })
        },
        {
...
def create_graph_data(controls, risks):
    ...
    for control_id in controls[:3]:  # First 3 controls
        ...
        existing = frappe.db.exists(
            ...
            {"entity_doctype": "Control Activity", "entity_id": control_id}
        )
        if not existing:
            doc = frappe.get_doc({
    ...
    for risk_id in risks[:3]:  # First 3 risks
        ...
        existing = frappe.db.exists(
            "Compliance Graph Entity",
            {"entity_doctype": "Risk Register Entry", "entity_id": risk_id}
        )
        if not existing:
            doc = frappe.get_doc({
                "doctype": "Compliance Graph Entity",
...
def create_risk_predictions(controls):
    ...
    for i, control_id in enumerate(controls[:3]):
        ...
        existing = frappe.db.exists(
            "Risk Prediction",
            {"control": control_id, "prediction_date": prediction_data["prediction_date"]}
        )
        if not existing:
            doc = frappe.get_doc({
