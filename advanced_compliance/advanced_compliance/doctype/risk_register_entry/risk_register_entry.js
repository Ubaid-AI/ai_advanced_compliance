...
frappe.ui.form.on("Risk Register Entry", {
    refresh(frm) {
        ...
        if (frm.doc.residual_risk_score) {
            ...
            frm.dashboard.set_headline_alert(
                `<span class="indicator ${level_colors[risk_level] || 'grey'}">
                    ${__("Risk Level")}: ${risk_level} (${__("Score")}: ${frm.doc.residual_risk_score})
                </span>`
            );
        }
        ...
        if (!frm.is_new()) {
            frm.add_custom_button(__("Link Control"), function() {
                frappe.prompt({
                    fieldtype: "Link",
                    ...
                    frm.save();
                }, __("Link Mitigating Control"), __("Add"));
            }, __("Actions"));
        }

        // Show heat map visualization
        if (frm.doc.inherent_risk_score && frm.doc.residual_risk_score) {
            const reduction = frm.doc.inherent_risk_score - frm.doc.residual_risk_score;
            if (reduction > 0) {
...
const _calculate_inherent_score = function(frm) {
    if (frm.doc.inherent_likelihood && frm.doc.inherent_impact) {
        const likelihood = parseInt(frm.doc.inherent_likelihood.split(" - ")[0]) || 0;
...
const _calculate_residual_score = function(frm) {
    if (frm.doc.residual_likelihood && frm.doc.residual_impact) {
        const likelihood = parseInt(frm.doc.residual_likelihood.split(" - ")[0]) || 0;
