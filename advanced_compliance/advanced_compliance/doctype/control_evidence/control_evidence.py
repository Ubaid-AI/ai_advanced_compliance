...
class ControlEvidence(Document):
    ...
    def generate_summary(self):
        ...
        summary_parts.append(
            _("Evidence captured for {0} {1}").format(
                self.source_doctype,
                self.source_name
            )
        )
        ...
        if self.workflow_log:
            try:
                ...
                summary_parts.append(
                    _("{0} workflow actions recorded").format(len(workflow_data))
                )
        ...
        if self.version_history:
            try:
                ...
                summary_parts.append(
                    _("{0} versions recorded").format(len(version_data))
                )
            except (json.JSONDecodeError, TypeError):
                pass
        ...
        if self.linked_documents:
            summary_parts.append(
                _("{0} linked documents captured").format(len(self.linked_documents))
            )
    ...
    def verify_integrity(self):
        ...
        if original_hash != new_hash:
            frappe.throw(
                _("Evidence integrity check failed. Evidence may have been tampered with.")
            )
    ...
    def get_evidence_for_control(control_activity, from_date=None, to_date=None):
        ...
        return frappe.get_all(
            ...
            filters=filters,
            fields=[
                "name", "source_doctype", "source_name",
                "captured_at", "document_snapshot", "evidence_summary"
            ],
