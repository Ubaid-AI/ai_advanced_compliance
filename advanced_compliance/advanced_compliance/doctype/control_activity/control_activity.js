    ...
    refresh(frm) {
        ...
        if (frm.doc.last_test_result) {
            ...
            frm.dashboard.set_headline_alert(
                `<span class="indicator ${result_colors[frm.doc.last_test_result] || 'grey'}">
        ...
        if (frm.doc.status === "Active" && !frm.is_new()) {
            frm.add_custom_button(__("Create Test Execution"), function() {
        ...
        if (!frm.is_new() && frm.doc.risks_addressed && frm.doc.risks_addressed.length > 0) {
        ...
        if (frm.doc.next_test_date && frappe.datetime.get_diff(frm.doc.next_test_date, frappe.datetime.nowdate()) < 0) {
            frm.dashboard.set_headline_alert(
                `<span class="indicator red">
                    ${__("Test Overdue!")} ${__("Due date was")} ${frm.doc.next_test_date}
                </span>`,
                "red"
            );
        }
