...
class NLQueryEngine:
    ...
    ENTITY_PATTERNS = {
        ...
        "owner": [
            r"(owned\s+by|assigned\s+to|belonging\s+to)\s+([a-zA-Z\s]+)",
            r"([a-zA-Z]+)'s\s+controls?"
        ],
        "status_value": [
            r"(active|inactive|draft|archived)",
            r"(open|closed|pending|resolved)"
        ],
        "control_type": [
            r"(preventive|detective|corrective)",
            r"(manual|automated|semi.?automated)"
        ]
    }
    ...
    def _build_filters(self, parsed):
        ...
        if "failed" in intents and doctype == "Test Execution":
            filters["test_result"] = ["in", ["Ineffective - Minor", "Ineffective - Significant", "Ineffective - Material"]]
        ...
        if "high_risk" in intents:
            if doctype == "Risk Register Entry":
                ...
                high_risk_threshold = frappe.db.get_single_value(
                    "Compliance Settings", "high_risk_threshold"
                )

                if not high_risk_threshold:
                    frappe.throw(
                        _("Please configure High Risk Threshold in Compliance Settings")
                    )
    ...
    def _execute_rule_based_query(self, parsed):
        ...
        if not doctype:
            return {
                "success": False,
                "error": _("Could not determine what to search for")
            }
        ...
        if not frappe.has_permission(doctype, "read"):
            return {
                "success": False,
                "error": _("No permission to access {0}").format(doctype)
            }
    ...
    def _get_display_fields(self, doctype):
        ...
        field_map = {
            "Control Activity": [
                "name", "control_name", "control_type", "status",
                "control_owner", "is_key_control", "automation_level",
                "last_test_date", "next_test_date"
            ],
            "Risk Register Entry": [
                "name", "risk_name", "risk_category",
                "risk_owner", "status", "inherent_risk_score", "residual_risk_score"
            ],
            "Deficiency": [
                "name", "description", "severity", "status", "control",
                "identified_date", "target_date", "remediation_owner"
            ],
            "Test Execution": [
                "name", "control", "test_date", "test_result",
                "tester", "conclusion"
            ]
        }
...
- limit: Result limit (default 50)"""
            ...
            try:
                ...
                frappe.log_error(
                    message=f"LLM returned invalid JSON: {response[:500]}",
                    title="NL Query Engine JSON Error"
                )
                return self._execute_rule_based_query(parsed)
            ...
            allowed_doctypes = [
                ...
                "Control Evidence", "Deficiency", "Compliance Alert", "Risk Prediction"
            ]
            if not doctype or doctype not in allowed_doctypes:
                frappe.log_error(
                    message=f"LLM returned invalid DocType: {doctype}",
                    title="NL Query Engine DocType Error"
                )
                return self._execute_rule_based_query(parsed)
            ...
            frappe.log_error(
                ...
                title="NL Query Engine LLM Error"
            )
...
Do not just say "Found X results" - provide insight about WHAT was found and WHY it matters."""
            ...
            frappe.log_error(
                message=f"AI Summary Generation Error: {str(e)}",
                title="NL Query AI Summary Error"
            )
    ...
    def _build_llm_context(self):
        ...
        doctypes = {
            "Control Activity": [
                "name", "control_name", "control_type", "status",
                "control_owner", "is_key_control", "automation_level", "frequency",
                "last_test_date", "next_test_date", "control_category"
            ],
            "Risk Register Entry": [
                "name", "risk_name", "risk_category",
                "risk_owner", "status", "inherent_risk_score", "residual_risk_score"
            ...
            "Deficiency": [
                ...
                "control", "identified_date", "target_date"
            ],
            "Test Execution": [
                "name", "control", "test_date", "test_result", "tester",
                "conclusion", "docstatus"
            ],
            "Compliance Framework": [
                "name", "framework_name", "description", "status"
            ],
            "Risk Prediction": [
                "name", "control", "control_name", "prediction_date",
                "failure_probability", "risk_level", "is_current",
    ...
    def _log_query(self, question, parsed, result):
        ...
        try:
            ...
            frappe.log_error(
                message=f"Failed to log NL query: {str(e)}",
                title="NL Query Log Error"
            )
