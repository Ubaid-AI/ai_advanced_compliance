...
from frappe.utils import nowdate, add_days, date_diff, flt, cint
import json
import time
...
class RiskPredictor:
    ...
    def predict(self, control_id):
        ...
        if not settings:
            frappe.throw(
                _("Please configure AI Provider Settings before running risk predictions")
            )
    ...
    def _get_days_since_test(self, control):
        ...
        if not settings:
            frappe.throw(
                _("Please configure Compliance Settings before running risk predictions")
            )
        ...
        if not default_days:
            frappe.throw(
                _("Control {0} has never been tested and no default value is configured. Please set 'Default Days for Never-Tested Controls' in Compliance Settings.").format(
                    frappe.bold(control.name)
                )
            )
    ...
    def _get_test_pass_rate(self, control_id):
        ...
        results = frappe.db.sql("""
            ...
            AND docstatus = 1
        """, {"control": control_id}, as_dict=True)
    ...
    def _get_test_count(self, control_id):
        ...
        return frappe.db.count(
            "Test Execution",
            {"control": control_id, "docstatus": 1}
        )
    ...
    def _get_open_deficiency_count(self, control_id):
        ...
        return frappe.db.count(
            "Deficiency",
            {"control": control_id, "status": ["not in", ["Closed", "Cancelled"]]}
        )
    ...
    def _calculate_probability(self, features):
        ...
        if test_pass_rate is not None:
            ...
            if not uncertainty_penalty:
                frappe.throw(
                    _("Please configure No Test History Penalty in AI Provider Settings")
                )
