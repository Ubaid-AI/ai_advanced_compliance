...
class AutoSuggest:
    ...
    def _get_linked_controls(self, risk_id):
        ...
        for rel in relationships:
            control_id = frappe.db.get_value(
                "Compliance Graph Entity",
                rel.source_entity,
                "entity_id"
            )
    ...
    def suggest_testing_priority(self, limit=10):
        ...
        controls = frappe.get_all(
            ...
            fields=[
                "name", "control_name", "is_key_control", "last_test_date",
                "test_frequency", "control_type", "automation_level"
            ]
        )
    ...
    def suggest_owner_for_control(self, control_id, limit=3):
        ...
        for user_id in user_ids:
            ...
            total_owned = frappe.db.count(
                "Control Activity",
                {"control_owner": user_id, "status": "Active"}
            )
            if total_owned < 5:
                score += 0.3
    ...
    def suggest_remediation(self, deficiency_id):
        ...
        suggestions["suggested_steps"] = severity_steps.get(
            deficiency.severity,
            severity_steps["Moderate"]
        )
