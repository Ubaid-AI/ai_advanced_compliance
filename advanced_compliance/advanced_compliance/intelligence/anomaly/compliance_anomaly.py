...
from frappe.utils import nowdate, add_days, date_diff, flt, cint, getdate
...
class ComplianceAnomalyDetector:
            ...
            if tc.test_count > threshold and tc.test_count > 5:
                anomalies.append({
                    ...
                    "recommended_action": _(
                        "Review testing schedule to ensure proper distribution"
                    )
                })
    ...
    def detect_pass_rate_changes(self, window_days=30):
        ...
        """, {"mid_date": mid_date, "start_date": start_date}, as_dict=True)
        ...
        for control in controls:
            ...
            if days_since > overdue_threshold:
                ...
                anomalies.append({
                    ...
                    "severity": severity,
                    "title": _("Overdue testing for {0}").format(
                        control.control_name or control.name
                    ),
                    "description": _(
                        "{0} days since last test (expected: every {1} days)"
                    ).format(days_since, expected_days),
                    "details": {
    ...
    def detect_deficiency_spikes(self, window_days=30):
        ...
        """, {"start_date": start_date, "mid_date": mid_date})[0][0]
        ...
        if increase_ratio >= spike_threshold and recent_count > 5:
            anomalies.append({
                ...
                "description": _("{0} deficiencies in last {1} days vs {2} in prior period ({3:.1f}x increase)").format(
                    recent_count, window_days, previous_count, increase_ratio
                ),
                "details": {
                ...
                "recommended_action": _(
                    "Investigate root cause of increased deficiencies"
                )
            })
    ...
    def detect_evidence_staleness(self, stale_days=180):
        ...
        """, {"stale_date": stale_date}, as_dict=True)
        ...
        for item in stale_evidence:
            ...
            anomalies.append({
                ...
                "severity": "Medium",
                "title": _("Stale evidence for {0}").format(
                    item.control_name or item.control_id
                ),
                "description": _(
                    "No evidence collected in {0} days"
                ).format(days_stale or "N/A (never collected)"),
                "details": {
                    "control_id": item.control_id,
                    "latest_evidence_date": str(item.latest_evidence_date) if item.latest_evidence_date else None,
