...
advanced_compliance = {
	...
	render_risk_heatmap: function(container, data) {
		...
		// Add click handlers
		$(container).find('.risk-cell').on('click', function() {
	...
	_render_control_details: function(control) {
		...
		// Status badge
		html += '<div class="mb-3">';
		html += '<span class="compliance-badge status-' + control.status.toLowerCase() + '">';
		html += control.status;
		html += '</span>';
		if (control.is_key_control) {
			html += ' <span class="compliance-badge status-warning">Key Control</span>';
		}
		html += '</div>';
		...
		html += '<p><strong>' + __("Owner") + ':</strong> ' + (control.control_owner || '-') + '</p>';
		html += '<p><strong>' + __("Type") + ':</strong> ' + (control.control_type || '-') + '</p>';
		html += '<p><strong>' + __("Frequency") + ':</strong> ' + (control.frequency || '-') + '</p>';
		html += '</div>';
		html += '<div class="col-md-6">';
		html += '<p><strong>' + __("Tests") + ':</strong> ' + (control.test_count || 0) + '</p>';
		html += '<p><strong>' + __("Open Deficiencies") + ':</strong> ' + (control.open_deficiencies || 0) + '</p>';
		html += '<p><strong>' + __("Last Test") + ':</strong> ' + (control.last_test_date || __("Never")) + '</p>';
		html += '</div>';
		html += '</div>';
	...
	show_nl_query_dialog: function() {
		...
		var d = new frappe.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					description: __("Enable for questions the rule-based engine can't handle")
				},
				{
		...
		var examples_html = '<div class="example-questions" style="margin-bottom: 15px;">';
		example_questions.forEach(function(q) {
			examples_html += '<button class="btn btn-xs btn-default example-q" style="margin: 3px;" data-question="' + q + '">';
			examples_html += q;
		...
		d.fields_dict.examples_html.$wrapper.find('.example-q').on('click', function() {
	...
	_execute_nl_query: function(dialog, question, use_llm) {
		...
		results_wrapper.html(
			...
			'<p class="text-muted">' + __("Searching...") + '</p>' +
			'</div>'
		);

		frappe.call({
			method: "advanced_compliance.advanced_compliance.intelligence.nlp.query_engine.ask_compliance_question",
			args: {
				question: question,
				use_llm: use_llm ? 1 : 0
			},
	...
	_render_nl_results: function(wrapper, result) {
		...
		if (result.query_type === 'count') {
			...
			html += '<h1 style="font-size: 48px; color: var(--primary);">' + result.count + '</h1>';
		...
		if (result.results && result.results.length > 0) {
			html += '<div class="frappe-list" style="max-height: 400px; overflow-y: auto;">';
			html += '<table class="table table-hover">';
			html += '<thead><tr>';

			// Get column headers from first result
			var columns = Object.keys(result.results[0]).filter(function(k) {
			...
			result.results.forEach(function(row) {
				html += '<tr class="result-row" data-doctype="' + result.doctype + '" data-name="' + row.name + '" style="cursor: pointer;">';
				columns.forEach(function(col) {
					var value = row[col];
					if (value === null || value === undefined) {
						...
						value = '<a href="/app/' + frappe.router.slug(result.doctype) + '/' + row.name + '">' + value + '</a>';
					}
					html += '<td>' + value + '</td>';
				});
				html += '</tr>';
			...
			if (result.count > result.results.length) {
				...
				html += '<a href="/app/' + frappe.router.slug(result.doctype) + '" class="btn btn-sm btn-default">';
